...
def wrangle(input: Path(), output: Path()) -> None:
    ...
    df['Observation Status'] = df.apply(lambda x: 'low' if 'suppressed' in str(x['Observation Status']) else x['Observation Status'], axis = 1 )
    df['Observation Status'] = df.apply(lambda x: 'x' if ('na' in str(x['Value'])) & ('low' not in str(x['Observation Status'])) else x['Observation Status'], axis = 1 )
